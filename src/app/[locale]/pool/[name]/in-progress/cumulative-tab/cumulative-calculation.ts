...
export const calculatePoolStats = (
...
): [Record<string, ParticipantsRoster> | null, TotalRanking[] | null] => {
  ...
  for (let i = 0; i < poolInfo.participants.length; i += 1) {
    ...
    for (
      let j = 0;
      j < poolInfo.context?.pooler_roster[participant].chosen_forwards.length;
      j += 1
    ) {
      const playerId =
    ...
    for (
      let j = 0;
      j < poolInfo.context?.pooler_roster[participant].chosen_defenders.length;
      j += 1
    ) {
      const playerId =
    ...
    for (
      let j = 0;
      j < poolInfo.context?.pooler_roster[participant].chosen_goalies.length;
      j += 1
    ) {
      const playerId =
      ...
      if (
        ...
        Object.keys(
          ...
          if (player) {
            ...
            stats[participantId].goalies[index].shutouts += player.SO ? 1 : 0;
  ...
  for (let i = 0; i < poolInfo.participants.length; i += 1) {
    ...
    // Find daily points made using the dailyPointsMade context.
